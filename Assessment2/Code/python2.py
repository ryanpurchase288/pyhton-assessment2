...
def one(input):
	input = list(input)
	output = [item for item in input for count in range(3)]
	return ''.join(output)
...
def four(input1, input2):
	output = ""
	for count in range(len(input1)):
		output += input1[count]+input2[count]
	return output
...
def seven(a, b, c):
	numList=[a,b,c]
	numList.sort()
	difference1=numList[1] - numList[0]
	difference2=numList[2] - numList[1]
...
def eight(input,  a):
	middle = (len(input)//2)
	value=a//2
	start = input[:middle-value] 
	end = input[middle+value+1:]
	output = start+end
	return output
